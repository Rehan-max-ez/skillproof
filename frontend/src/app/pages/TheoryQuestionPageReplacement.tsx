...
export function TheoryQuestionPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const submitted = await submitAnswer(session, currentQuestion.question_id, answer);
      const evaluation = await evaluateResponse(session, submitted.response_id);

      updateAssessment({
        currentResponseId: submitted.response_id,
      });
